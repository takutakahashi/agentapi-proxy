...
// TestBuildSessionSettings_GitHubAppTokenGeneratedServerSide verifies that when a
// GitHub App auth Secret is configured (no per-request personal token), the proxy
// generates a short-lived installation token server-side and injects only
// GITHUB_TOKEN into the session Pod — never the GitHub App configuration or PEM.
func TestBuildSessionSettings_GitHubAppTokenGeneratedServerSide(t *testing.T) {
	...
	// Override the resolver so the test never performs a real GitHub API call and
	// verifies that the resolved repository is forwarded correctly.
	var gotRepo string
	manager.githubTokenResolver = func(repoFullName string) (string, error) {
		gotRepo = repoFullName
		return "ghs_installationtoken", nil
	}
	...
	settings, err := manager.buildSessionSettings(context.Background(), session, req, nil)
	if err != nil {
		t.Fatalf("buildSessionSettings() error = %v", err)
	}

	if gotRepo != "octo/repo" {
		t.Fatalf("resolver received repo %q, want octo/repo", gotRepo)
	}
	if got := settings.Env["GITHUB_TOKEN"]; got != "ghs_installationtoken" {
		t.Fatalf("GITHUB_TOKEN = %q, want ghs_installationtoken", got)
	}
	for _, key := range githubAppSecretKeys {
		if _, ok := settings.Env[key]; ok {
			t.Fatalf("session env must not contain GitHub App key %q, got value present", key)
		}
	}
