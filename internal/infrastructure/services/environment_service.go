...
// LocalEnvironmentService implements the EnvironmentService interface
type LocalEnvironmentService struct {
	roleEnvFiles *config.RoleEnvFilesConfig
}

// NewLocalEnvironmentService creates a new LocalEnvironmentService
func NewLocalEnvironmentService(roleEnvFiles *config.RoleEnvFilesConfig) *LocalEnvironmentService {
	return &LocalEnvironmentService{
		roleEnvFiles: roleEnvFiles,
	}
}
...
// 2. Auth team environment file (from team_role_mapping)
// 3. Team/organization specific environment file (from tags["env_file"])
// 4. Role-based environment variables
func (s *LocalEnvironmentService) MergeEnvironmentVariables(ctx context.Context, cfg *services.EnvMergeConfig) (map[string]string, error) {
	mergedEnv := make(map[string]string)

	// 1. Load role-based environment variables (lowest priority)
	if s.roleEnvFiles != nil && s.roleEnvFiles.Enabled && cfg.UserRole != "" {
		envVars, err := config.LoadRoleEnvVars(s.roleEnvFiles, cfg.UserRole)
		if err != nil {
	...
	if cfg.RequestEnv != nil {
		validatedEnv := s.validateEnvironmentVariables(cfg.RequestEnv)
...
func (s *LocalEnvironmentService) ExtractTeamEnvFile(tags map[string]string) string {
	if tags == nil {
		return ""
	}
...
func (s *LocalEnvironmentService) validateEnvironmentVariables(env map[string]string) map[string]string {
