...
func (r *Router) registerConditionalRoutes() error {
	...
	if r.handlers.googleOAuthController != nil {
		log.Printf("[ROUTES] Registering scia OAuth integration endpoints...")
		...
		r.echo.GET("/integrations/notion-oauth/status", r.handlers.googleOAuthController.GetNotionStatus, auth.RequirePermission(entities.PermissionSessionRead, r.server.container.AuthService))
		log.Printf("[ROUTES] scia OAuth integration endpoints registered")
	}
