...
func setupProcessEnv(env map[string]string) map[string]string {
	...
	for k, v := range env {
		...
		case "HTTP_PROXY", "HTTPS_PROXY", "http_proxy", "https_proxy",
			...
			"AGENTAPI_SCIA_PROXY_URL", "AGENTAPI_SCIA_GOOGLE_CREDENTIAL", "AGENTAPI_SCIA_NOTION_CREDENTIAL", "AGENTAPI_SCIA_USER_NAMESPACE", "AGENTAPI_SCIA_NOTION_USER_NAMESPACE":
			continue
		default:
			filtered[k] = v
		}
