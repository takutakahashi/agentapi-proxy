...
func TestGitHubAuthProvider_CustomRolePriority(t *testing.T) {
	tests := []struct {
		name            string
		teams           []GitHubTeamMembership
		config          config.GitHubUserMapping
		...
		{
			name: "custom role priority - superadmin wins",
			teams: []GitHubTeamMembership{
				{
					Organization: "test-org",
					TeamSlug:     "dev-team",
			...
			config: config.GitHubUserMapping{
				...
				TeamRoleMapping: map[string]config.TeamRoleRule{
					"test-org/dev-team": {
						Role:        "developer",
						Permissions: []string{"write", "read"},
						EnvFile:     "/etc/agentapi/envs/dev.env",
					},
					"test-org/super-team": {
						Role:        "superadmin",
						Permissions: []string{"admin", "write", "read"},
						EnvFile:     "/etc/agentapi/envs/super.env",
					},
				},
			},
			expectedRole:    "superadmin",
			expectedEnvFile: "/etc/agentapi/envs/super.env",
		},
		{
			name: "undefined role in priority - no upgrade",
			teams: []GitHubTeamMembership{
				{
					Organization: "test-org",
					TeamSlug:     "custom-team",
					Role:         "member",
				},
			},
			config: config.GitHubUserMapping{
				...
				TeamRoleMapping: map[string]config.TeamRoleRule{
					"test-org/custom-team": {
						Role:        "customrole", // Not defined in priority
						Permissions: []string{"custom"},
						EnvFile:     "/etc/agentapi/envs/custom.env",
					},
				},
			},
			expectedRole:    "user", // Should stay as default since customrole is not in priority
			expectedEnvFile: "",     // No env file since role didn't upgrade
		},
	}
