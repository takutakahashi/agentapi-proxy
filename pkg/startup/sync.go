...
func syncMarketplaces(opts SyncOptions, settings *settingsJSON) error {
	...
	// Fall back to the (optional) external settings file if the compile-generated one is not yet available.
	settingsPath := filepath.Join(claudeDir, "settings.json")
	compileSettings, err := readClaudeSettingsJSON(settingsPath)
	if err != nil {
		log.Printf("[SYNC] Warning: could not read marketplaces from %s: %v", settingsPath, err)
		compileSettings = &claudeSettingsJSON{}
	}

	// Merge marketplaces: compile-generated settings take precedence; external settings (opts.SettingsFile)
	// are also consulted so that custom marketplace URLs are always available.
...
func readEnabledPluginsFromSettingsJSON(settingsPath string) ([]string, error) {
	content, err := readClaudeSettingsJSON(settingsPath)
	if err != nil {
		return nil, err
	}
