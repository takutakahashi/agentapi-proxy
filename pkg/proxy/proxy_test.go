...
func TestNewProxyFixed(t *testing.T) {
	...
	if proxy.GetConfig() != cfg {
		t.Error("Proxy config not set correctly")
	}

	if proxy.GetEcho() == nil {
...
// TestExtractRepoFullNameFromURLFixed was moved to internal/app/server_test.go
// as the function is now internal to that package
