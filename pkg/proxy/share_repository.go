...
type MemoryShareRepository struct {
	mu             sync.RWMutex
	shares         map[string]*SessionShare // token -> share
	sessionToToken map[string]string        // sessionID -> token
}
...
func (r *MemoryShareRepository) Save(share *SessionShare) error {
	...
	if oldToken, exists := r.sessionToToken[share.SessionID()]; exists {
		delete(r.shares, oldToken)
	}

	r.shares[share.Token()] = share
	r.sessionToToken[share.SessionID()] = share.Token()
	return nil
}
...
func (r *MemoryShareRepository) FindBySessionID(sessionID string) (*SessionShare, error) {
	...
	token, exists := r.sessionToToken[sessionID]
	if !exists {
...
func (r *MemoryShareRepository) Delete(sessionID string) error {
	...
	delete(r.sessionToToken, sessionID)
	return nil
}
...
// Save persists a session share
func (r *KubernetesShareRepository) Save(share *SessionShare) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *KubernetesShareRepository) FindByToken(token string) (*SessionShare, error) {
	...
	return NewSessionShareWithToken(sj.Token, sj.SessionID, sj.CreatedBy, sj.CreatedAt, sj.ExpiresAt), nil
}
