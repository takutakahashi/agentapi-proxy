...
type Handlers struct {
	...
	githubController *WebhookGitHubController
	customController *WebhookCustomController
}
...
func NewHandlers(repo repositories.WebhookRepository, sessionManager repositories.SessionManager, baseURL string, memoryRepo repositories.MemoryRepository, sessionProfileRepo repositories.SessionProfileRepository) *Handlers {
	...
	return &Handlers{
		...
		githubController: NewWebhookGitHubController(repo, sessionManager, memoryRepo, sessionProfileRepo),
		customController: NewWebhookCustomController(repo, sessionManager, memoryRepo, sessionProfileRepo),
	}
}
...
func (h *Handlers) triggerGitHubWebhook(ctx echo.Context, webhook *entities.Webhook, req *TriggerWebhookRequest) error {
	if req.Event == "" {
	...
	response := TriggerWebhookResponse{
		DryRun:  req.DryRun,
	...
	response.MatchedTrigger = &TriggerMatchedTriggerInfo{
		ID:   matchResult.ID(),
	...
	if req.DryRun {
		...
		dryResult, err := h.githubController.SessionService().DryRunSessionConfig(SessionCreationParams{
			Webhook:        webhook,
			Trigger:        matchResult,
			Payload:        req.Payload,
			Tags:           tags,
	...
	sessionID, sessionReused, err := h.githubController.SessionService().CreateSessionFromWebhook(ctx.Request().Context(), SessionCreationParams{
		Webhook:        webhook,
		Trigger:        matchResult,
		Payload:        req.Payload,
		RawPayload:     nil,
...
func (h *Handlers) triggerCustomWebhook(ctx echo.Context, webhook *entities.Webhook, req *TriggerWebhookRequest) error {
	// Match triggers
	...
	if req.DryRun {
		dryResult, err := h.customController.SessionService().DryRunSessionConfig(SessionCreationParams{
			Webhook:        webhook,
			Trigger:        matchResult,
			Payload:        req.Payload,
			Tags:           tags,
	...
	payloadBytes, _ := json.Marshal(req.Payload)
	sessionID, sessionReused, err := h.customController.SessionService().CreateSessionFromWebhook(ctx.Request().Context(), SessionCreationParams{
		Webhook:        webhook,
		Trigger:        matchResult,
		Payload:        req.Payload,
